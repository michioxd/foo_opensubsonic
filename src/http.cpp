...
response open(const char *url, abort_callback &abort,
  ...
  FB2K_console_formatter() << "[foo_opensubsonic][http] response status="
                           << (out.status_text.is_empty()
                                   ? "<unavailable>"
...
void read_all(file::ptr stream, mem_block_container &out, abort_callback &abort,
  ...
  for (;;) {
    abort.check();
    const size_t got = stream->read(buffer.get_ptr(), actual_chunk_size, abort);
    if (got == 0) {
      break;
    }
