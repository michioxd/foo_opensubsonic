...
[[nodiscard]] pfc::string8 hash_artwork_bytes(const void *data, t_size size) {
  const auto hash = static_api_ptr_t<hasher_md5>()->process_single(data, size);
...
bool try_read_artwork_file(const char *path, album_art_data_ptr &out_data,
  ...
  if (path == nullptr || *path == '\0' || !filesystem::g_exists(path, abort)) {
    return false;
  }
...
bool ensure_artwork_cached(const subsonic::cached_track_metadata &track_meta,
  ...
  if (mime_type.is_empty()) {
    (void)subsonic::http::try_get_header(response, "content-type", mime_type);
  }
...
public:
  ...
  album_art_data_ptr query(const GUID &art_id, abort_callback &abort) override {
