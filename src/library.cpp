...
build_artist_sync_plan(const subsonic::server_credentials &credentials,
  ...
  for (size_t offset = 0;; offset += k_album_list_page_size) {
    ...
    if (album_list_it == album_list_root.end() || !album_list_it->is_object()) {
      break;
    }

    std::vector<album_sync_summary> page_albums;
    for_each_member_item(*album_list_it, "album", [&](const json &album_node) {
  ...
  for (size_t i = 0; i < artist_plans.size(); ++i) {
    ...
    for (size_t album_index = 0; album_index < artist.albums.size();
      ...
      set_library_fetch_status(status, i + 1, artist_plans.size(),
                               ...
                               artist_track_index, artist.total_track_count);

      const json album_root = fetch_endpoint(
          credentials, "getAlbum.view",
          make_query_params(
              {subsonic::query_param(pfc::string8("id"), album.album_id)}),
          abort);
...
[[nodiscard]] std::vector<remote_playlist_sync_result> fetch_remote_playlists(
  ...
  for (size_t i = 0; i < summaries.size(); ++i) {
    ...
    for_each_member_item(*playlist_it, "entry", [&](const json &entry_node) {
      ...
      merge_unique_metadata(playlist_metadata_entries, metadata_index, entry);
    });
...
[[nodiscard]] t_size ensure_playlist(playlist_manager &api,
  ...
  if (index == SIZE_MAX) {
    throw std::runtime_error(PFC_string_formatter()
                             << "Unable to create playlist '" << desired_name
                             << "'.");
  }
  ...
  if (api_v2 != nullptr) {
    try {
      ...
      if (remote_id != nullptr && *remote_id != '\0') {
        api_v2->playlist_set_property(index, guid_playlist_property_remote_id,
      ...
      subsonic::log_warning("library",
                            "unknown error while setting playlist properties");
    }
  }
    ...
    if (!remove) {
      ...
      if (api.playlist_get_name(playlist_index, name)) {
        remove =
            ...
            subsonic::starts_with_ascii_nocase(name, k_remote_playlist_prefix);
      }
    }
...
[[nodiscard]] sync_outcome perform_sync(sync_mode mode,
  ...
  try {
    ...
    throw std::runtime_error(PFC_string_formatter()
                             << "sync stage ensure_cache_layout failed: " << e);
  }
  ...
  if (mode == sync_mode::library_only || mode == sync_mode::all) {
    ...
    subsonic::metadata::replace_track_metadata(outcome.library.entries, status,
                                               abort);
  }
...
void apply_sync_outcome(const sync_outcome &outcome) {
  ...
  if (outcome.includes_playlists) {
    ...
    for (const auto &playlist : outcome.playlists) {
      ...
      const auto playlist_index =
          ensure_playlist(*playlist_api, playlist_api_v2_raw, display_name,
                          k_remote_playlist_scope, playlist.remote_id);
      replace_playlist_items(*playlist_api, playlist_index, playlist.handles);
    }
  }
}
...
public:
  ...
  void on_done(HWND p_wnd, bool p_was_aborted) override {
    ...
    try {
      ...
      popup_message::g_show(PFC_string_formatter() << "Apply sync failed:\n"
                                                   << e.what(),
                            "foo_opensubsonic");
    }
  }
...
public:
  ...
  void on_done(HWND p_wnd, bool p_was_aborted) override {
    ...
    if (!m_error_msg.is_empty()) {
      popup_message::g_show(PFC_string_formatter() << "Clear cache failed:\n"
                                                   << m_error_msg,
                            "foo_opensubsonic");
      return;
    }
...
public:
  ...
  bool get_description(t_uint32 index, pfc::string_base &out) override {
    switch (index) {
    case cmd_sync_library:
      out = "Fetches the remote OpenSubsonic catalog into the local cache and "
    ...
    case cmd_sync_all:
      ...
      return true;
    case cmd_clear_cache:
      out = "Clears cached metadata/artwork and removes managed playlists.";
      return true;
    default:
      return false;
    }
