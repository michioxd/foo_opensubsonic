...
bool content_type_looks_like_error_payload(const char *content_type) noexcept {
  return subsonic::starts_with_ascii_nocase(content_type, "text/xml") ||
         subsonic::starts_with_ascii_nocase(content_type, "application/xml") ||
         subsonic::starts_with_ascii_nocase(content_type, "application/json") ||
...
public:
  ...
  size_t lowLevelIO(const GUID &guid, size_t arg1, void *arg2, size_t arg2size,
...
private:
  void reopen_at(t_filesize offset, abort_callback &abort) {
    auto response = open_download_response(m_credentials, m_track_id, abort);
...
public:
  ...
  void move(const char *src, const char *dst, abort_callback &abort) override {
    (void)src;
    (void)dst;
    abort.check();
    throw exception_io_denied_readonly();
  ...
  void extract_filename_ext(const char *path, pfc::string_base &out) override {
    out = make_display_name_for_path(path);
  }
